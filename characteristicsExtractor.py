...
if __name__ == '__main__':
    ...
    parser.add_argument("fileXorGenGen")
    parser.add_argument("fileXorGenForg")
    ...
    ##        p.map(funcsWrapper,files)
    ##    for i in charVecList:
    ##        charDict1[getFile(i[0])] = i[1:]
    ##    json.dump(charDict1,open(args.path+args.fileGen+'.characteristics','w+'))            
##
    ...
    ##        p.map(funcsWrapper,files)
    ##    for i in charVecList:
    ##        charDict2[getFile(i[0])] = i[1:]
    ##    json.dump(charDict2,open(args.path+args.fileForg+'.characteristics','w+'))  
    
    for i in [args.fileXorGenGen,args.fileXorGenForg]:
        with open(args.path+i) as f:
            ...
            json.dump(charDict3,open(args.path+i+'.characteristicsCentered','w+'))
